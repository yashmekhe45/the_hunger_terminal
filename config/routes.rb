...
Rails.application.routes.draw do
  ...
  get 'review/previous_review' => 'reviews#previous_review' ,:as => 'previous_review'
  resource :reviews, only: [:create,:destroy,:edit]
  ...
  namespace :api do
    api_version(:module => "v1", :header => {:name => "Accept", :value => "application/vnd.hunger-terminal.com; version=1"}) do
      ...
      match '/terminals/:terminal_id/placing_orders' => 'order_placing#order_bill', :via => :post
      match '/orders/myOrders' => 'order_details#order_history', :via => :post
    end
  end
